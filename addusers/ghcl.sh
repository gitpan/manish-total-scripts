...
# 1.    Adds a user with prepend ghcl-
...
do
	ctr=`expr $ctr + 1`
	/usr/sbin/adduser ghcl-$i -g ghclindia -s /bin/false
	echo password |passwd --stdin ghcl-$i
	cd /home/ghcl-$i/
	maildirmake Maildir
	...
	mail $i@ghclindia.com -s "Test Mail from HECL ISGCS" < /scripts/isg_scripts/addusers/firstmail.txt	
         cp /scripts/isg_scripts/addusers/.qmail_default /home/ghcl-$i/.qmail
