...
do
	...
	if [ $? -ne 0 ]; then
		...
		ctr=`expr $ctr + 1`
		/usr/sbin/adduser raj-$i -g idldwge -s /bin/false
		echo password | passwd --stdin raj-$i
		cd /home/raj-$i/
		maildirmake Maildir
		...
		/usr/sbin/setquota -u -n raj-$i /dev/ida/c0d0p5 5120 5120 0 0
		echo $i@dwge.com >> /var/qmail/control/relaymailfrom-221.171.85.12
		cp /scripts/isg_scripts/addusers/.qmail_dwge /home/raj-$i/.qmail
