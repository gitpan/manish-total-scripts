...
mailq |grep -v done|grep remote|cut -d "@" -f 2|sort > mail-q
uniq mail-q > uniq-domain
echo > pending_remote_status

for i in `cat uniq-domain`
do
echo -e `fgrep -cx $i mail-q`'\t' pending for '\t'$i >> pending_remote_status
done

sort -nr pending_remote_status|mail -s "Pending Mail status remote delivery" -c isgidc@hughes-ecomm.com 

#rm mail-q
#rm uniq-domain
#rm pending_remote_status


#for i in `mailq|grep remote|grep -v done|cut -d "@" -f 2|sort|uniq`
#do
#	echo `mailq|grep -v done|grep remote|grep $i|wc -l` "   "  $i >> /mgulati/pending_mailstatus_countwise
#
#done
#
