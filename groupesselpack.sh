...
expr="/usr/bin/expr"
wc="/usr/bin/wc"
used=`repquota -a -g | grep esselpack | head -1 | awk '{ print $3 } '`
soft=`repquota -a -g | grep esselpack | head -1 | awk '{ print $4 } '`
hard=`repquota -a -g | grep esselpack | head -1 | awk '{ print $5 } '`
msgbytes=`$cat - | $wc -c`
